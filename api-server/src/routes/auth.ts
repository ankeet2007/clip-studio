...
async function saveToken(token: TokenData): Promise<void> {
  ...
  // Awaited so the token is durably stored before returning to the caller.
  try {
...
async function pollForToken(): Promise<void> {
  try {
    ...
    if (error) {
      // Only genuinely transient errors should silently retry.
      // Fatal errors (invalid_client, access_denied, invalid_grant, …) will never
      // resolve — stop polling so the UI can surface them immediately.
